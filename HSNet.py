...
def _expand_token(token, batch_size):
    return token.expand(batch_size, -1).unsqueeze(1)  # 形状变为 [batch_size, 1, embedding_dim]
def cross_modal_feats_vit_l14(img,backbone):
    bsz,c,h,w = img.shape
    x = backbone.conv1(img)  # shape = [*, width, grid, grid]
    width,height = x.shape[0], x.shape[1]
    x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
    x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
    x = torch.cat([_expand_token(backbone.class_embedding, x.shape[0]).to(x.dtype), x], dim=1)
    # shape = [*, grid ** 2 + 1, width]
    x = x + backbone.positional_embedding.to(x.dtype)
    x = backbone.patch_dropout(x)
    x = backbone.ln_pre(x)
    x = backbone.transformer(x)

    x = backbone.ln_post(x)
    pooled, tokens = backbone._global_pool(x)
    if backbone.proj is not None:
        pooled = pooled @ backbone.proj
        tokens = tokens @ backbone.proj
    tokens = tokens.reshape(bsz, 16, 16, 768)
    return tokens

def extract_feat_modified_res(img, backbone, feat_ids, bottleneck_ids, layer_ids):
    r""" Extract intermediate features from ModifiedResNet"""
    ...
    for hid, (bid, lid) in enumerate(zip(bottleneck_ids, layer_ids)):
        ...
        feat = block.conv1.forward(feat)
        feat = block.bn1.forward(feat)
        feat = block.act1.forward(feat)
        feat = block.conv2.forward(feat)
        feat = block.bn2.forward(feat)
        feat = block.act2.forward(feat)

        
        feat = block.avgpool.forward(feat)

        feat = block.conv3.forward(feat)
        feat = block.bn3.forward(feat)

        if bid == 0:  # Downsample if needed
            if hasattr(block, 'downsample'):
                res = block.downsample.forward(res)

        feat += res
        
        if hid+1 in feat_ids:
            feats.append(feat.clone())
        feat = block.act3.forward(feat)
    image_embedding,image_features = backbone.attnpool(feat)
    image_features = image_features.view(7, 7, image_features.size(1), image_features.size(2))
    image_features = image_features.permute(2, 3, 0, 1)
    feats.append(image_features.clone())
