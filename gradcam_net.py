...
class text_aux_net(nn.Module):
    def __init__(self, backbone, shot, use_original_imgsize):
        super(text_aux_net, self).__init__()
    ...
    def forward(self, query_img, support_img, support_mask,gradcam):
        with torch.no_grad():
        ...
        levels = [2]*len(query_feats)
        for i in range(self.stack_ids[0] + 1):
            levels[i] = 0
        for i in range(self.stack_ids[0] + 1,self.stack_ids[1] + 1):
            levels[i] = 1
        fusion_query_protos = []
...
if __name__ == "__main__":
    batch = {
        ...
        'grad_cams':torch.rand(4,16,16)
    }
    model = text_aux_net('resnet50', 5, False)
    out = model(batch['query_img'], batch['support_imgs'],batch['support_masks'],batch['grad_cams'])
    print(out.size())